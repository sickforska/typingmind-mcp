...
async function startClient(clientId, config) {
  ...
  const transport = new StdioClientTransport({
    command,
    args,
    env: Object.values(env).length > 0 ? env : undefined, // cannot be {}, it will cause error
  });
...
async function start(authToken) {
  ...
  app.post('/start', async (req, res) => {
    try {
      ...
      // Remove clients that are not in the new config
      const currentIds = Array.from(clients.keys());
      const newIds = Object.keys(mcpServers);
      const toRemove = currentIds.filter((id) => !newIds.includes(id));

      for (const id of toRemove) {
        try {
          await clients.get(id).client.close();
          clients.delete(id);
        } catch (e) {
          console.error('Error removing old client', id, e);
        }
      }
  ...
  app.get('/clients/:id/tools', async (req, res) => {
    const { id } = req.params;
    const clientEntry = clients.get(id);
  ...
  app.post('/clients/:id/call_tools', async (req, res) => {
    const { id } = req.params;
